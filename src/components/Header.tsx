      ...
      <Container className="!px-0">
        <nav className="shadow-md md:shadow-none bg-white md:bg-transparent mx-auto flex justify-between items-center py-2 px-5 md:py-10">
          ...
          <div className="md:hidden flex items-center">
            ...
            >
              {isOpen ? <HiOutlineXMark className="h-6 w-6" aria-hidden="true" fill="#fff" stroke="#fff" /> : <HiBars3 className="h-6 w-6" aria-hidden="true" fill="#fff" stroke="#fff" />}
      ...
      <Transition
        ...
        <div id="mobile-menu" className="md:hidden bg-white shadow-lg">
          <ul className="flex flex-col space-y-4 pt-1 pb-6 px-6">
            {menuItems.map((item) => (
              <li key={item.text}>
                <Link href={item.url} className="text-foreground hover:text-primary transition-colors block" onClick={toggleMenu}>
                  {item.text}
                </Link>
              </li>
            ))}
